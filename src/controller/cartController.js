...
const createCart = async function (req, res) {
    try {
        ...
        if (!userCart) {
            ...
            if (!product) return res.status(400).send({ status: false, message: "No product found" })
        ...
        if (userCart) {
            ...
            if (!cart) return res.status(400).send({ status: false, message: "No cart found with this ID" })
